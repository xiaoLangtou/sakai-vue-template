...
* **Do *not* run production build commands** inside the agent session:
...
| `npm run build:pro`        | **Production build – *do not run during agent sessions*** |
| `npm run build:pro:rsdoctor` | **Production build with analysis – *do not run during agent sessions*** |
