...
export function useErrorHandling() {
  ...
  const initializePage = async (
    criticalRequests: Array<{ url: string; config?: RequestConfig }>,
    optionalRequests: Array<{ url: string; config?: RequestConfig }> = []
  ) => {
    try {
      // 关键请求必须成功
      const criticalPromises = criticalRequests.map(({ url, config }) => 
        http.get(url, config)
      );
      
      // 可选请求失败不影响页面渲染
      const optionalPromises = optionalRequests.map(({ url, config }) => 
        safeRequest(url, config)
      );
