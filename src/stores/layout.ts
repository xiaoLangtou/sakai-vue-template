...
export const useLayoutStore = defineStore('layout', () => {
    ...
    /** 是否为桌面端 */
    const isDesktop = computed(() => windowWidth.value >= 1024 && windowWidth.value <= 1439);
    /** 是否为宽屏桌面端 */
    ...
    const handleResize = () => {
        ...
        const wasTablet = prevWidth >= 768 && prevWidth <= 1023;
        const wasDesktop = prevWidth >= 1024 && prevWidth <= 1439;
        const wasWide = prevWidth >= 1440;

        // 当从移动端或平板端切换到桌面端或宽屏时
        if ((wasMobile || wasTablet) && (isDesktop.value || isWide.value)) {
            // 关闭移动端/平板端侧边栏
        ...
        // 当从桌面端或宽屏切换到移动端或平板端时
        else if ((wasDesktop || wasWide) && (isMobile.value || isTablet.value)) {
            // 关闭移动端/平板端侧边栏
            if (showMobileSidebar.value) {
                showMobileSidebar.value = false;
            }
