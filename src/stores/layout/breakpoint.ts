...
export function useBreakpoint() {
     ...
     * 处理断点变化时的状态调整
     */
    const handleBreakpointChange = (prevBreakpoint: BreakpointType, newBreakpoint: BreakpointType) => {
        const wasMobileOrTablet = prevBreakpoint === BreakpointType.MOBILE || prevBreakpoint === BreakpointType.TABLET;
        const isDesktopOrWide = newBreakpoint === BreakpointType.DESKTOP || newBreakpoint === BreakpointType.WIDE;
        ...
        // 从桌面端/宽屏切换到移动端/平板端时
        else if ((prevBreakpoint === BreakpointType.DESKTOP || prevBreakpoint === BreakpointType.WIDE) && (newBreakpoint === BreakpointType.MOBILE || newBreakpoint === BreakpointType.TABLET)) {
            // 这里的逻辑需要在主 store 中处理，因为涉及到其他状态
            // 暂时留空，由调用方处理
        }
    };
