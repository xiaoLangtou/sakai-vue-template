...
<template>
    <PageContainer>
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <div>
                    ...
                    <Button label="批量删除" icon="pi pi-trash" severity="danger" :disabled="selectedMenus.length === 0"
        ...
        <!-- 搜索区域 -->
        <ListSearch :filterConfigs="filterConfigs" placeholder="请输入搜索内容" :loading="loading" v-model="searchParams"
            @search="handleSearch">
            <template #actions>
                <Button icon="pi pi-refresh" class="mr-2" severity="secondary" outlined @click="refreshMenus"
                    :disabled="loading" />
            </template>
        </ListSearch>
        ...
        <Divider type="dashed" />
        <!-- TreeTable -->
        <TreeTable v-model:selection="selectedMenus" :value="treeData" size="normal" :resizableColumns="true"
            showGridlines :loading="loading" dataKey="key" :scrollable="true" scrollHeight="600px">
            <!-- 菜单名称列 -->
            ...
            <Column field="type" header="类型" style="width: 120px">
                <template #body="{ node }">
                    <Tag :value="getTypeConfig(node.data.type).label"
            ...
            <Column field="path" header="路由路径" style="width: 200px">
                <template #body="{ node }">
                    <a v-if="node.data.isExternal && node.data.externalUrl" :href="node.data.externalUrl"
            ...
            <Column field="status" header="状态" style="width: 100px; text-align: center">
                <template #body="{ node }">
                    <Tag :value="node.data.status === 1 ? '启用' : '禁用'"
            ...
            <Column field="visible" header="显示" style="width: 80px; text-align: center">
                <template #body="{ node }">
                    <Tag :value="node.data.visible ? '是' : '否'"
            ...
            <Column header="操作" style="width: 200px">
                <template #body="{ node }">
                    <div class="flex gap-2">
                        <Button icon="pi pi-plus" size="small" class="p-button-text p-button-success"
                            v-tooltip="'添加子菜单'" @click="handleAddChild(node.data.id)" />
                        <Button icon="pi pi-pencil" size="small" class="p-button-text" v-tooltip="'编辑'"
                            @click="handleEdit(node.data.id)" />
                        <Button icon="pi pi-trash" size="small" class="p-button-text p-button-danger" v-tooltip="'删除'"
        ...
        <!-- 菜单表单弹窗 -->
        <MenuForm v-model:visible="dialogVisible" :title="dialogTitle" v-model:form-data="formData"
