...
const performSearch = async (query: string) => {
    ...
    try {
        ...
        const results = mockSearchData.filter((item) => item.title.toLowerCase().includes(query.toLowerCase()) || item.description?.toLowerCase().includes(query.toLowerCase()) || item.category?.toLowerCase().includes(query.toLowerCase()));
...
<template>
    <Dialog :visible="visible" modal :header="undefined" :style="{ width: '640px', maxWidth: '90vw' }" :closable="false" :draggable="false" class="global-search-dialog" @hide="closeDialog">
        <div class="global-search-container">
            <!-- 搜索框 -->
            <div class="search-input-section">
                <div class="search-input-wrapper">
                    <Search class="search-icon" :size="18" />
                    <InputText ref="inputRef" v-model="searchQuery" :placeholder="placeholder" class="search-input" @input="handleSearchInput" @keydown.enter.prevent />
                    <div class="search-shortcuts">
                        <kbd v-if="!hasSearchQuery" class="shortcut-key">⌘K</kbd>
                        <Button v-if="hasSearchQuery" icon="pi pi-times" class="clear-btn" text rounded size="small" @click="resetSearch" />
                    </div>
                </div>
            </div>
                ...
                <ScrollPanel class="search-scroll" style="width: 100%; height: 400px">
                    ...
                    <div v-if="hasSearchQuery" class="search-results">
                        ...
                        <div v-else-if="hasResults" class="results-list">
                            <div v-for="(result, index) in searchResults" :key="result.id" class="result-item" :class="{ selected: index === selectedIndex }" @click="selectResult(result)" @mouseenter="selectedIndex = index">
