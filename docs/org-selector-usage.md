...
```vue
<template>
    <OrgSelector :data="orgData" :multiple="true" :only-select-person="true" @change="handleSelectionChange" />
</template>
...
<template>
    <div>
        ...
        <OrgSelector v-model:visible="selectorVisible" :data="orgData" mode="modal" title="选择团队成员" :multiple="true" :only-select-person="true" @confirm="handleConfirm" @cancel="handleCancel" />
    </div>
</template>
...
```vue
<template>
    <OrgSelector :data="orgData" :multiple="true" :only-select-person="false" @change="handleMixedSelection">
        <template #header>
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-semibold">选择部门或人员</h3>
                <div class="text-sm text-gray-500">已选择: {{ selectedCount }} 项</div>
            </div>
        </template>
    </OrgSelector>
</template>
...
```vue
<template>
    <OrgSelector :data="orgData" :multiple="false" :only-select-person="true" @change="handleSingleSelection" />
</template>
...
```vue
<template>
    <OrgSelector :data="filteredOrgData" :multiple="true" @change="handleSelectionChange">
        <template #search>
            <div class="p-4 border-b">
                <div class="flex gap-2 mb-2">
                    <InputText v-model="searchKeyword" placeholder="搜索人员或部门..." class="flex-1" />
                    <Dropdown v-model="filterDepartment" :options="departmentOptions" option-label="name" option-value="id" placeholder="筛选部门" class="w-48" />
                </div>
                <div class="flex gap-2">
                    <Tag v-for="tag in activeTags" :key="tag.id" :value="tag.label" removable @remove="removeTag(tag)" />
                </div>
            </div>
        </template>
    </OrgSelector>
...
<template>
    <div>
        ...
        <OrgSelector ref="orgSelectorRef" :data="orgData" @change="handleChange" />
    </div>
</template>
...
<template>
    <div>
        ...
        <OrgSelector v-else :data="orgData" :loading="loading" />
    </div>
</template>
```
